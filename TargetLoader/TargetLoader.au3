...
Func RenderWindow()
	...
	$hGUI = GUICreate("TargetLoader", $labelWidth + 60, 780, $iGUILeft, $iGUITop)
	$Btn_Start = GUICtrlCreateDummy()
	For $i = 0 To UBound($aTargetNames) - 1
		$x = $i * 30 + 50
		$hLabel = GUICtrlCreateLabel(StringFormat("%-15s", $aTargetNames[$i]) & @TAB & $aTargetRas[$i] & @TAB & $aTargetDes[$i] & @TAB & $aTargetReps[$i] & " × " & $aTargetExps[$i] & " sec" & @TAB & $aTargetTime[$i], 40, $x, $labelWidth, 20)
		_ArrayAdd($aLabelCtls, $hLabel)
		GUICtrlSetCursor($hLabel, 0)
		GUICtrlSetFont($hLabel, 8.5, $FW_BOLD, 0, "Courier New")
	Next
	$Btn_End = GUICtrlCreateDummy()

	Local $idButton_Reload  = GUICtrlCreateButton("reload", 5, 5, 80, 20)
				...
				If $lastLabel <> $aInfo[4] Then
					For $i = 0 To UBound($aLabelCtls) - 1
						If $aLabelCtls[$i] == $aInfo[4] Then
							GUICtrlSetFont($aLabelCtls[$i], 8.5, $FW_BOLD, $GUI_FONTUNDER, "Courier New")
						Else
							GUICtrlSetFont($aLabelCtls[$i], 8.5, $FW_BOLD, 0, "Courier New")
						EndIf
					Next
					$lastLabel = $aInfo[4]
				EndIf
...
While 1
	...
	If $aLabelData <> "" Then FillMaxImDL()
WEnd
